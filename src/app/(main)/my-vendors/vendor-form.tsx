    ...
    async function fetchCategories() {
        ...
        if (snapshot.exists()) {
            ...
            snapshot.forEach(child => {
                const val = child.val();
                cats.push({ id: child.key!, name: val.name });
            })
            setCategories(cats);
        }
    }
  ...
  const onSubmit: SubmitHandler<VendorFormValues> = async (data) => {
    ...
    try {
      ...
      const dataToSave = {
        ...
        imageId: data.imageId || `https://picsum.photos/seed/${newVendorRef.key}/400/300`,
      }

      await set(newVendorRef, dataToSave);
                    ...
                    render={({ field }) => (
                        <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                            ...
                            <div className="space-y-1 leading-none">
                                ...
                                This is a private listing, so this won't have a public effect.
                                </p>
                                <FormMessage />
                            </div>
                        </FormItem>
